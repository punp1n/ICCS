...
The repository centers on three tracks. `CNP/` stores raw Codificación Penal DOCX/XLSX per period plus `procesar_consolidado.py`, which consolidates the newest code definition into parquet/xlsx under the same folder; `CNP/BD/` holds the SQL Server loaders (`cargar_*_sqlserver.py`) and DDL (`modelo_tablas_*.sql`). `Correspondencia automatica/` is a numbered pipeline: `1_iccs/generar_iccs_tabla.py` and `1_iccs/generar_iccs_descripcion.py` (with `parse_defs/*.csv`) generate `1_iccs/outputs/iccs_tabla.*` / `1_iccs/outputs/iccs_descripcion.*` from `ICSS_PDF/*.pdf`; then `2_embeddings/` and `3_llm_filter/` consume them; `ENUSC/` is an auxiliary branch. Each stage keeps its results in its own `outputs/` folder. Manual adjudications live under `Correspondencia manual/<año>/`, while `ICCS_UNODC/` and `ICSS_PDF/` host reference material. Avoid editing generated `.xlsx/.parquet/.csv` directly; regenerate them via the scripts.
