...
PDF_PATH = Path(
    r"C:\\Users\\Asvaldebenitom\\OneDrive - Instituto Nacional de Estadisticas\\Seguridad y justicia\\ICCS\\ICCS_SPANISH_2016_web.pdf"
)
OUTPUT_DIR = Path(
...
PARSE_DEFS_DIR = OUTPUT_DIR / "parse_defs"
START_PAGE_INDEX = 25  # pagina 26 (0-indexado)
END_PAGE_INDEX = 33  # pagina 34 (0-indexado)
MAX_SECTION = 11
OUTPUT_BASENAME = "iccs_parse_final"  # Updated output basename
...
def extract_section_data() -> List[Dict[str, Any]]:
    ...
    def finalize_current() -> None:
        ...
        code_len = len(code)
        entry = {
            "codigo_iccs": int(code), # Add cod for join
            "nivel_1": int(code[:2]),
            "nivel_2": int(code[:4]),
...
def save_outputs(df: pd.DataFrame, base_name: str) -> None:
    ...
    df.to_excel(base_path.with_suffix(".xlsx"), index=False, sheet_name="iccs_completa")
...
def main() -> None:
    ...
    # 2. Load data from parse_defs CSVs
    df_csv = load_parse_defs_data()

    # 3. Merge the two dataframes
    df_merged = pd.merge(df_pdf, df_csv, on="codigo_iccs", how="left")

    # 4. Compare descriptions
    df_merged["glosa_match"] = df_merged.apply(
        lambda row: _normalize_string_for_comparison(row["delito_iccs"]) == \
                    _normalize_string_for_comparison(row["glosa_iccs"]),
        axis=1
    )

    # 5. Save the final dataframe
    final_columns = [
        "codigo_iccs", "delito_iccs", "glosa_iccs", "glosa_match",
        "nivel_1", "nivel_2", "nivel_3", "nivel_4", "seccion", 
        "descripcion", "inclusiones", "exclusiones", "notas"
    ]
    # Reorder columns to have the most important ones first
